...
## ---------------------------- command --derep_id and mandatory output

## --derep_id is accepted
DESCRIPTION="--derep_id is accepted"
...
## --derep_id requires --output
DESCRIPTION="--derep_id requires --output"
printf ">s\nAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        2> /dev/null && \
...
DESCRIPTION="--derep_id accepts empty input"
printf "" | \
    "${VSEARCH}" \
        --derep_id - \
        --output /dev/null 2> /dev/null && \
...
DESCRIPTION="--derep_id rejects fastq input"
printf "@s\nA\n+\nI\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --output /dev/null 2> /dev/null && \
...
DESCRIPTION="--derep_id rejects non-fasta input (#1)"
printf "\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --output /dev/null 2> /dev/null && \
...
DESCRIPTION="--derep_id rejects non-fasta input (#2)"
printf "\n>s\nAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --output /dev/null 2> /dev/null  && \
...
DESCRIPTION="--derep_id accepts a single fasta entry"
printf ">s\nAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --output - 2> /dev/null | \
...
DESCRIPTION="--derep_id discards a short fasta entry"
printf ">s\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --output - 2> /dev/null | \
...
DESCRIPTION="--derep_id discards an empty fasta entry"
printf ">s\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --output - 2> /dev/null | \
...
DESCRIPTION="--derep_id compare two empty fasta entries"
printf ">s1\n>s2\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --output /dev/null 2> /dev/null && \
...
# attempt to trigger a special case of seqcmp()
DESCRIPTION="--derep_id compare two fasta entries (one is empty)"
printf ">s1\nA\n>s1\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --output /dev/null 2> /dev/null && \
...
DESCRIPTION="--derep_id outputs stderr messages"
printf ">s\nAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --output /dev/null 2>&1 | \
...
DESCRIPTION="--derep_id --quiet removes stderr messages"
printf ">s\nAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --quiet \
...
DESCRIPTION="--derep_id --minseqlength 1 (keep very short fasta entries)"
printf ">s\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id --minseqlength 0 (keep empty fasta entries)"
printf ">s\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 0 \
...
## --derep_id outputs data
DESCRIPTION="--derep_id outputs data"
printf ">s\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --quiet \
...
## --derep_id outputs expected results
DESCRIPTION="--derep_id outputs expected results (in fasta format)"
printf ">s1\nA\n>s1\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id requires identical sequence labels (different labels)"
printf ">s1\nA\n>s2\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id requires identical sequence labels (same labels)"
printf ">s1\nA\n>s1\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id requires identical sequence labels (same labels, same annotations)"
printf ">s1;size=1\nA\n>s1;size=1\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
## abundance annotations are part of sequence labels?
DESCRIPTION="--derep_id requires identical sequence labels (same labels, different annotations)"
printf ">s1;size=1\nA\n>s1;size=2\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
## --derep_id replicate sequences are not sorted by
## alphabetical order of headers. Identical sequences receive the
## header of the first sequence of their group (s2 before s1)
DESCRIPTION="--derep_id identical seqs receive the header of the first seq of the group"
printf ">s1\nA\n>s1\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
## --derep_id distinct sequences are sorted by
## alphabetical order of headers (s1 before s2)
DESCRIPTION="--derep_id distinct sequences are sorted by header alphabetical order"
printf ">s2\nA\n>s1\nG\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
## --derep_id distinct sequences are not sorted by
## alphabetical order of DNA strings (G before A)
DESCRIPTION="--derep_id distinct sequences are not sorted by DNA alphabetical order"
printf ">s2\nA\n>s1\nG\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id sort clusters by decreasing abundance (natural order)"
printf ">s1;size=3\nA\n>s2;size=1\nC\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id sort clusters by decreasing abundance (reversed input order)"
printf ">s1;size=1\nA\n>s2;size=3\nC\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id then sort clusters by comparing headers (natural order)"
printf ">s1;size=2\nA\n>s2;size=2\nC\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id then sort clusters by comparing headers (reversed input order)"
printf ">s2;size=2\nA\n>s1;size=2\nC\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id then sort clusters by input order (natural order)"
printf ">s1;size=2\nC\n>s1;size=2\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id then sort clusters by input order (reversed input order)"
printf ">s1;size=2\nA\n>s1;size=2\nC\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
## --derep_id sequence comparison is case insensitive
DESCRIPTION="--derep_id sequence comparison is case insensitive"
printf ">s1\nA\n>s1\na\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
## --derep_id preserves the case of the first occurrence of each sequence
DESCRIPTION="--derep_id preserves the case of the first occurrence of each sequence"
printf ">s1\na\n>s1\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
## --derep_id T and U are considered the same
DESCRIPTION="--derep_id T and U are considered the same"
printf ">s1\nT\n>s1\nU\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
## --derep_id does not replace U with T in its output
DESCRIPTION="--derep_id does not replace U with T in its output"
printf ">s1\nU\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
## --derep_id accepts more than 1,024 unique sequences
## (trigger reallocation)
DESCRIPTION="--derep_id accepts more than 1,024 unique sequences"
(for i in {1..1025} ; do
    printf ">s%d\n" "${i}"
    yes A | head -n "${i}"
 done) | \
...
DESCRIPTION="--derep_id outputs a median cluster size"
printf ">s1\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id median (empty entry, no median)"
printf "" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id median (single entry, median = 1)"
printf ">s1\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id median (single entry with size annotation, median = 2)"
printf ">s1;size=2\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id median (two entries, median = 1)"
printf ">s1\nA\n>s2\nC\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id median (two entries with equal size annotations, median = 2)"
printf ">s1;size=2\nA\n>s2;size=2\nC\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id median (1 + 2 -> median = 2)"
printf ">s1;size=1\nA\n>s2;size=2\nC\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id median (1 + 3 -> median = 2)"
printf ">s1;size=1\nA\n>s2;size=3\nC\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id median (1 + 4 -> median = 2)"
printf ">s1;size=1\nA\n>s2;size=4\nC\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id median (1 + 5 -> median = 3)"
printf ">s1;size=1\nA\n>s2;size=5\nC\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id median (1 + 6 -> median = 4)"
printf ">s1;size=1\nA\n>s2;size=6\nC\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id median (1 + 7 -> median = 4)"
printf ">s1;size=1\nA\n>s2;size=7\nC\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id median (1 + 8 -> median = 4)"
printf ">s1;size=1\nA\n>s2;size=8\nC\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id median (1 + 9 -> median = 5)"
printf ">s1;size=1\nA\n>s2;size=9\nC\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id rejects compressed stdin (bzip2)"
printf ">s\nA\n" | \
    bzip2 | \
    "${VSEARCH}" \
        --derep_id - \
...
DESCRIPTION="--derep_id --bzip2_decompress rejects uncompressed stdin"
printf ">s\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --bzip2_decompress \
...
DESCRIPTION="--derep_id --fasta_width is accepted"
printf ">s\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id fasta output is not wrapped (80 nucleotides or less)"
printf ">s\n%80s\n" | tr " " "A" | \
    "${VSEARCH}" \
        --derep_id - \
        --quiet \
...
DESCRIPTION="--derep_id fasta output is wrapped (81 nucleotides or more)"
printf ">s\n%81s\n" | tr " " "A" | \
    "${VSEARCH}" \
        --derep_id - \
        --quiet \
...
DESCRIPTION="--derep_id --fasta_width is accepted (empty input)"
printf "" | \
    "${VSEARCH}" \
        --derep_id - \
        --fasta_width 80 \
...
DESCRIPTION="--derep_id --fasta_width 2^32 is accepted"
...
DESCRIPTION="--derep_id --fasta_width 1 (1 nucleotide per line)"
printf ">s\nTT\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 2 \
...
DESCRIPTION="--derep_id --fasta_width 0 (no wrapping)"
printf ">s\n%81s\n" | tr " " "A" | \
    "${VSEARCH}" \
        --derep_id - \
        --fasta_width 0 \
...
DESCRIPTION="--derep_id rejects compressed stdin (gzip)"
printf ">s\nA\n" | \
    gzip | \
    "${VSEARCH}" \
        --derep_id - \
...
DESCRIPTION="--derep_id --gzip_decompress accepts uncompressed stdin"
printf ">s\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --gzip_decompress \
...
DESCRIPTION="--derep_id rejects --bzip2_decompress + --gzip_decompress"
printf "" | \
    "${VSEARCH}" \
        --derep_id - \
        --bzip2_decompress \
...
DESCRIPTION="--derep_id --lengthout adds length annotations to output"
printf ">s\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id --lengthout --sizeout add annotations to output (size first)"
printf ">s\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id --log is accepted"
printf ">s\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id --log writes to a file"
printf ">s\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id --log does not prevent messages to be sent to stderr"
printf ">s\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id accepts empty input (0 unique sequences)"
printf "" | \
    "${VSEARCH}" \
        --derep_id - \
        --output /dev/null \
...
DESCRIPTION="--derep_id --maxseqlength is accepted"
printf ">s\n%81s\n" | tr " " "A" | \
    "${VSEARCH}" \
        --derep_id - \
        --maxseqlength 81 \
...
DESCRIPTION="--derep_id --maxseqlength accepts shorter lengths (<)"
printf ">s\n%80s\n" | tr " " "A" | \
    "${VSEARCH}" \
        --derep_id - \
        --maxseqlength 81 \
...
DESCRIPTION="--derep_id --maxseqlength accepts equal lengths (=)"
printf ">s\n%81s\n" | tr " " "A" | \
    "${VSEARCH}" \
        --derep_id - \
        --maxseqlength 81 \
...
DESCRIPTION="--derep_id --maxseqlength rejects longer sequences (>)"
printf ">s\n%82s\n" | tr " " "A" | \
    "${VSEARCH}" \
        --derep_id - \
        --maxseqlength 81 \
...
DESCRIPTION="--derep_id --maxseqlength accepts shorter lengths (--log)"
printf ">s\n%80s\n" | tr " " "A" | \
    "${VSEARCH}" \
        --derep_id - \
        --maxseqlength 81 \
...
DESCRIPTION="--derep_id --maxseqlength discards longer lengths (--log)"
printf ">s\n%80s\n" | tr " " "A" | \
    "${VSEARCH}" \
        --derep_id - \
        --maxseqlength 79 \
...
DESCRIPTION="--derep_id --maxseqlength must be an integer"
printf ">s\n%81s\n" | tr " " "A" | \
    "${VSEARCH}" \
        --derep_id - \
        --maxseqlength A \
...
DESCRIPTION="--derep_id --minseqlength is accepted"
printf ">s\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
# note: the 'sequence discarded' message is not silenced by --quiet
DESCRIPTION="--derep_id --minseqlength rejects shorter sequences (<)"
printf ">s\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 2 \
...
DESCRIPTION="--derep_id --minseqlength accepts equal lengths (=)"
printf ">s\nAA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 2 \
...
DESCRIPTION="--derep_id --minseqlength accepts longer sequences (>)"
printf ">s\nAAA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id --minseqlength accepts longer sequences (--log)"
printf ">s\nAA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id --minseqlength discards short sequences (--log)"
printf ">s\nAA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 3 \
...
DESCRIPTION="--derep_id --minseqlength must be an integer"
printf ">s\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength A \
...
DESCRIPTION="--derep_id --minseqlength must be a positive integer"
printf ">s\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength -1 \
...
DESCRIPTION="--derep_id --minseqlength --maxseqlength (equal)"
printf ">s\nAA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 2 \
...
DESCRIPTION="--derep_id --minseqlength --maxseqlength (swapped threshold)"
printf ">s\nAA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 2 \
...
DESCRIPTION="--derep_id --no_progress is accepted"
printf ">s\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id --no_progress removes progressive report on stderr (no visible effect)"
printf ">s\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id --notrunclabels is accepted"
printf ">s\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id --notrunclabels preserves full headers"
printf ">s extra\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id --quiet is accepted"
printf ">s\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id --quiet eliminates all (normal) messages to stderr"
printf ">s extra\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id --quiet allows error messages to be sent to stderr"
printf ">s extra\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id --relabel renames sequence (empty label, only ticker)"
printf ">s\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id --relabel cannot combine with --relabel_md5"
printf ">s\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id --relabel cannot combine with --relabel_sha1"
printf ">s\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id --relabel_keep is accepted"
printf ">s\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id --relabel_keep renames and keeps original sequence name"
printf ">s\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id --relabel_md5 is accepted"
printf ">s\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id --relabel_md5 relabels using MD5 hash of sequence"
printf ">s\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id --relabel_self is accepted"
printf ">s\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id --relabel_self relabels using sequence as label"
printf ">s\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id --relabel_sha1 is accepted"
printf ">s\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id --relabel_sha1 relabels using SHA1 hash of sequence"
printf ">s\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id --sample is accepted"
printf ">s\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id --sample adds sample name to sequence headers"
printf ">s\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id --sizeout is accepted (no size)"
printf ">s\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id --sizeout is accepted (with size)"
printf ">s;size=2\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id --sizeout missing size annotations are not added (no size)"
printf ">s\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id size annotations are replaced (without sizein, with sizeout)"
printf ">s;size=2\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id size annotations are replaced (with sizein and sizeout)"
printf ">s;size=2\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id size annotations are left untouched (without sizein and sizeout)"
printf ">s;size=2\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id --relabel --sizeout adds size annotations"
printf ">s\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id --relabel_self --sizeout adds size annotations"
printf ">s\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id --relabel_md5 --sizeout adds size annotations"
printf ">s\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id --relabel_sha1 --sizeout adds size annotations"
printf ">s\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id --relabel --sizeout updates size annotations (without sizein)"
printf ">s;size=2\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id --relabel --sizeout updates size annotations (with sizein)"
printf ">s;size=2\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id --relabel_self --sizeout updates size annotations (without sizein)"
printf ">s;size=2\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id --relabel_self --sizeout preserves size annotations (with sizein)"
printf ">s;size=2\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id --relabel_md5 --sizeout updates size annotations"
printf ">s;size=2\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id --relabel_md5 --sizeout preserves size annotations (with sizein)"
printf ">s;size=2\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id --relabel_sha1 --sizeout updates size annotations"
printf ">s;size=2\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id --relabel_sha1 --sizeout preserves size annotations (with sizein)"
printf ">s;size=2\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id --threads is accepted"
printf ">s\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id --threads > 1 triggers a warning (not multithreaded)"
printf ">s\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id --xee is accepted"
printf ">s;ee=1.00\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id --xee removes expected error annotations from input"
printf ">s;ee=1.00\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id --xlength is accepted"
printf ">s;length=1\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id --xlength removes length annotations from input"
printf ">s;length=1\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id --xlength accepts input without length annotations"
printf ">s\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
DESCRIPTION="--derep_id --xlength removes length annotations (input), lengthout adds them (output)"
printf ">s;length=2\nA\n" | \
    "${VSEARCH}" \
        --derep_id - \
        --minseqlength 1 \
...
if which valgrind > /dev/null 2>&1 ; then
    ...
    valgrind \
        ...
        --uc /dev/null \
        --output /dev/null 2> /dev/null
    DESCRIPTION="--derep_id valgrind (no leak memory)"
    grep -q "in use at exit: 0 bytes" "${TMP}" && \
        success "${DESCRIPTION}" || \
            failure "${DESCRIPTION}"
    DESCRIPTION="--derep_id valgrind (no errors)"

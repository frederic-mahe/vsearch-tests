...
DESCRIPTION="--fastx_subsample errors if unable to open output file for writing (fasta)"
...
DESCRIPTION="--fastx_subsample errors if unable to open output file for writing (fastq)"
TMP=$(mktemp) && chmod u-w "${TMP}"  # remove write permission
printf "@s\nA\n+\nI\n" | \
    "${VSEARCH}" \
        --fastx_subsample - \
...
DESCRIPTION="--fastx_subsample errors if unable to open output file for writing (--fastaout_discarded)"
TMP=$(mktemp) && chmod u-w "${TMP}"  # remove write permission
printf ">s\nA\n" | \
    "${VSEARCH}" \
        --fastx_subsample - \
...
DESCRIPTION="--fastx_subsample errors if unable to open output file for writing (--fastqout_discarded)"
TMP=$(mktemp) && chmod u-w "${TMP}"  # remove write permission
printf "@s\nA\n+\nI\n" | \
    "${VSEARCH}" \
        --fastx_subsample - \
...
# --maxseqlength/--minseqlength were rejected by fastx_subsample until
# the 2026-08-15 documentation audit, although the 50000-nt default
# cap was silently applied by the shared database reader; the two
# tests below fail against vsearch 2.31.0 and older
DESCRIPTION="--fastx_subsample --maxseqlength retains sequences longer than 50000"
...
DESCRIPTION="--fastx_subsample --minseqlength is accepted"
printf ">s\nACGT\n" | \
    "${VSEARCH}" \
        --fastx_subsample - \
        --sample_size 1 \
...
"${VSEARCH}" \
    --fastx_subsample "${AT_DIR}/sampleA_R1.fasta" \
    --sample_size 1 \
    --quiet \
    --relabel @ \
    --fastaout /dev/null 2> /dev/null && \
...
DESCRIPTION="--fastx_subsample --relabel @ renames after the input file"
AT_DIR=$(mktemp -d)
printf ">s\nA\n" > "${AT_DIR}/sampleA_R1.fasta"
"${VSEARCH}" \
    --fastx_subsample "${AT_DIR}/sampleA_R1.fasta" \

...
DESCRIPTION="--shuffle errors if unable to open output file for writing"
...
"${VSEARCH}" \
    --shuffle "${AT_DIR}/sampleA_R1.fasta" \
    --quiet \
    --relabel @ \
    --output /dev/null 2> /dev/null && \
    success "${DESCRIPTION}" || \
...
DESCRIPTION="--shuffle --relabel @ renames after the input file"
AT_DIR=$(mktemp -d)
printf ">s\nA\n" > "${AT_DIR}/sampleA_R1.fasta"
"${VSEARCH}" \
    --shuffle "${AT_DIR}/sampleA_R1.fasta" \
...
DESCRIPTION="--shuffle --relabel no size annotations (size annotation in, without --sizeout)"
printf ">s;size=2\nA\n" | \
    "${VSEARCH}" \
        --shuffle - \
        --quiet \
...
DESCRIPTION="--shuffle --relabel_self no size annotations (size annotation in, without --sizeout)"
printf ">s;size=2\nA\n" | \
    "${VSEARCH}" \
        --shuffle - \
        --quiet \
...
DESCRIPTION="--shuffle --relabel_md5 no size annotations (size annotation in, without --sizeout)"
printf ">s;size=2\nA\n" | \
    "${VSEARCH}" \
        --shuffle - \
        --quiet \
...
DESCRIPTION="--shuffle --relabel_sha1 no size annotations (size annotation in, without --sizeout)"
printf ">s;size=2\nA\n" | \
    "${VSEARCH}" \
        --shuffle - \
        --quiet \

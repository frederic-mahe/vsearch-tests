...
DESCRIPTION="--sortbylength errors if unable to open output file for writing"
...
DESCRIPTION="--sortbylength orders labels that share an 8-byte prefix"
${VSEARCH} \
    --sortbylength <(printf ">AAAAAAAAXb;size=1\nT\n>AAAAAAAAXa;size=1\nA\n") \
    --quiet \
    --output - | \
    tr -d "\n" | \
...
DESCRIPTION="--sortbylength --topn rejects a negative value"
INPUT=$(mktemp)
printf ">s1;size=3\nAA\n" > "${INPUT}"
"${VSEARCH}" \
    --sortbylength "${INPUT}" \
...
"${VSEARCH}" \
    --sortbylength "${AT_DIR}/sampleA_R1.fasta" \
    --quiet \
    --relabel @ \
    --output /dev/null 2> /dev/null && \
    success "${DESCRIPTION}" || \
...
DESCRIPTION="--sortbylength --relabel @ renames after the input file"
AT_DIR=$(mktemp -d)
printf ">s\nA\n" > "${AT_DIR}/sampleA_R1.fasta"
"${VSEARCH}" \
    --sortbylength "${AT_DIR}/sampleA_R1.fasta" \

...
"${VSEARCH}" \
    --sortbysize "${AT_DIR}/sampleA_R1.fasta" \
    --quiet \
    --relabel @ \
    --output /dev/null 2> /dev/null && \
    success "${DESCRIPTION}" || \
...
DESCRIPTION="--sortbysize --relabel @ renames after the input file"
AT_DIR=$(mktemp -d)
printf ">s;size=1\nA\n" > "${AT_DIR}/sampleA_R1.fasta"
"${VSEARCH}" \
    --sortbysize "${AT_DIR}/sampleA_R1.fasta" \
...
DESCRIPTION="--sortbysize --relabel no size annotations (size annotation in, without --sizeout)"
printf ">s;size=2\nA\n" | \
    "${VSEARCH}" \
        --sortbysize - \
        --quiet \
...
DESCRIPTION="--sortbysize --relabel_self no size annotations (size annotation in, without --sizeout)"
printf ">s;size=2\nA\n" | \
    "${VSEARCH}" \
        --sortbysize - \
        --quiet \
...
DESCRIPTION="--sortbysize --relabel_md5 no size annotations (size annotation in, without --sizeout)"
printf ">s;size=2\nA\n" | \
    "${VSEARCH}" \
        --sortbysize - \
        --quiet \
...
DESCRIPTION="--sortbysize --relabel_sha1 no size annotations (size annotation in, without --sizeout)"
printf ">s;size=2\nA\n" | \
    "${VSEARCH}" \
        --sortbysize - \
        --quiet \
